...
const sceneSequence_1 = require("./lib/sceneSequence");
class GiraEndpointAdapter extends utils.Adapter {
    ...
    constructor(options = {}) {
        ...
        this.warnedMissingSubscriptions = new Set();
        ...
        this.failedSubscriptionMeta = new Set();
        ...
        this.messageArchiveWriteRunning = new Set();
        this.sceneKeys = [];
        this.sequenceKeys = [];
        this.sceneDescMap = new Map();
        this.sequenceDescMap = new Map();
    ...
    async onReady() {
        try {
            ...
            for (const key of this.sceneKeys)
                await this.createSceneObjects(key);
            for (const key of this.sequenceKeys)
                await this.createSequenceObjects(key);
            ...
            const validSceneBases = new Set(this.sceneKeys.map((key) => `SC@.${(0, sceneSequence_1.sanitizeSceneId)(key)}`));
            const validSequenceBases = new Set(this.sequenceKeys.map((key) => `SQ@.${(0, sceneSequence_1.sanitizeSequenceId)(key)}`));
            ...
            this.client.on("open", () => {
                ...
                this.fetchedMeta = new Set();
                this.failedSubscriptionMeta = new Set();
                this.metaQueue = new coMetaQueue_1.CoMetaRequestQueue(async (key) => this.fetchMeta(key, this.keyIdMap.get(key) ?? this.makeEndpointBaseId(key)), this.fetchedMeta, coMetaQueue_1.CO_META_MAX_CONCURRENCY, this.failedSubscriptionMeta);
                this.skipInitialUpdate = new Set(this.initialSkipUpdate);
                const subscriptionKeys = (0, messageArchive_1.buildSubscriptionKeys)([...this.endpointKeys, ...this.sceneKeys, ...this.sequenceKeys], this.messageArchives.map((archive) => archive.key));
                ...
                for (const key of this.sceneKeys)
                    void this.initializeScene(key);
                for (const key of this.sequenceKeys)
                    void this.initializeSequence(key);
                if (this.messageArchives.length) {
                    for (const archive of this.messageArchives) {
                        void this.readMessageArchive(archive).catch((err) => {
            ...
            this.client.on("close", (info) => {
                ...
                for (const key of this.sceneKeys)
                    this.setState(`${this.sceneBase(key)}.subscription`, false, true);
                for (const key of this.sequenceKeys)
                    this.setState(`${this.sequenceBase(key)}.subscription`, false, true);
            });
            ...
            this.client.on("event", async (payload) => {
                ...
                if (!data)
                    return;
                if (payload.type !== "subscribe" && await this.handleSpecialEvent(payload))
                    return;
                ...
                if (payload.type === "subscribe" && typeof data === "object" && Array.isArray(data.items)) {
                    ...
                    for (const item of data.items) {
                        ...
                        if ((0, messageArchive_1.isMessageArchiveKey)(key, messageArchiveKeys)) {
                            const archive = this.messageArchives.find((candidate) => candidate.key.toLowerCase() === String(key).toLowerCase());
                            if (archive) {
                                const baseId = `MA@.${this.sanitizeArchiveId(archive.key)}`;
                                const items = (0, messageArchive_1.getMessageArchiveEventItems)(item.data ?? item);
                                if (items.length) {
                                    await this.setStateAsync(`${baseId}.items`, { val: JSON.stringify(items), ack: true });
                                    await this.updateLastMessageArchiveStates(baseId, items);
                                }
    ...
    sceneBase(key) { return `SC@.${(0, sceneSequence_1.sanitizeSceneId)(key)}`; }
    sequenceBase(key) { return `SQ@.${(0, sceneSequence_1.sanitizeSequenceId)(key)}`; }
    isSpecialKey(key) { return /^SC@/i.test(key) || /^SQ@/i.test(key); }
    async createSceneObjects(key) {
        ...
        for (const action of [...Object.keys(sceneSequence_1.SCENE_ACTION_METHODS), "refreshActors"]) {
        ...
        for (const [suffix, common] of states)
            await this.setObjectNotExistsAsync(`${base}.${suffix}`, { type: "state", common, native: {} });
        await this.setStateAsync(`${base}.subscription`, { val: false, ack: true });
    }
    async createSequenceObjects(key) {
    ...
    async specialCall(key, method) {
        try {
            const response = await this.client.call(key, method, undefined, this.makeTag(`${key.slice(0, 2).toLowerCase()}_${method}`));
            await this.setStateAsync(`${/^SC@/i.test(key) ? this.sceneBase(key) : this.sequenceBase(key)}.status`, { val: (0, GiraClient_1.codeToMessage)(response?.code ?? 0), ack: true });
            return response;
        }
        catch (err) {
    ...
    async initializeSequence(key) {
        ...
        try {
            ...
            if (typeof metadata?.restart === "boolean")
                await this.setStateAsync(`${base}.restartAllowed`, { val: metadata.restart, ack: true });
            await this.applySpecialData(key, state?.data ?? state);
        }
        catch { /* the individual call already reports the error */ }
    }
    async setSpecialSubscription(rawKey, success, code) {
        const key = /^SC@/i.test(rawKey) ? (0, sceneSequence_1.normalizeSceneKey)(rawKey) : (0, sceneSequence_1.normalizeSequenceKey)(rawKey);
        ...
        await this.setStateAsync(`${base}.status`, { val: (0, GiraClient_1.codeToMessage)(code ?? 0), ack: true });
        if (success && /^SQ@/i.test(key))
            void this.initializeSequence(key);
    }
    async applySpecialData(rawKey, data) {
        const key = /^SC@/i.test(rawKey) ? (0, sceneSequence_1.normalizeSceneKey)(rawKey) : (0, sceneSequence_1.normalizeSequenceKey)(rawKey);
        const timestamp = (0, sceneSequence_1.extractTimestamp)(data);
        if (/^SC@/i.test(key)) {
            ...
            if (running !== undefined)
                await this.setStateAsync(`${this.sequenceBase(key)}.running`, { val: running, ack: true });
            if (timestamp !== undefined)
                await this.setStateAsync(`${this.sequenceBase(key)}.lastChange`, { val: timestamp, ack: true });
        }
    }
    async handleSpecialEvent(payload) {
        const rawKey = payload?.subscription?.key ?? payload?.data?.key ?? payload?.data?.uid;
        if (!rawKey || !this.isSpecialKey(String(rawKey)))
            return false;
    ...
    onStateChange(id, state) {
        ...
        if (mapped && this.handleMappedStateChange(id, state, mapped))
            return;
        if (this.handleMessageArchiveStateChange(id, state))
            return;
        if (this.handleSceneSequenceStateChange(id, state))
            return;
    ...
    handleSceneSequenceStateChange(id, state) {
        ...
        if (scene) {
            ...
            const method = action === "refreshActors" ? "get_items" : sceneSequence_1.SCENE_ACTION_METHODS[action];
            if (!method)
                return false;
            void this.specialCall(scene, method).then(async (response) => {
                if (method === "get_items")
                    await this.setStateAsync(`${this.sceneBase(scene)}.actors`, { val: JSON.stringify(response?.data ?? response), ack: true });
                if (method === "learn")
        ...
        if (sequence) {
            ...
            const method = sceneSequence_1.SEQUENCE_ACTION_METHODS[action];
            if (!method)
                return false;
    ...
    handleMessageArchiveStateChange(id, state) {
        if (!id.startsWith("MA@."))
            return false;
        if (state.ack)
            return true;
        ...
        if (!archive || (action !== "read" && action !== "testWrite"))
            return true;
        ...
        if (action === "read") {
            ...
            void this.runExperimentalMessageArchiveWrite(archive);
        return true;
    }
    async readMessageArchive(archive) {
        const baseId = `MA@.${this.sanitizeArchiveId(archive.key)}`;
    ...
    async updateLastMessageArchiveStates(baseId, items) {
        const state = (0, messageArchive_1.getLastMessageArchiveState)(items);
        if (!state)
            return;
        if (state.key !== undefined)
            await this.setStateAsync(`${baseId}.lastMessage.key`, { val: state.key, ack: true });
        if (state.text !== undefined)
            await this.setStateAsync(`${baseId}.lastMessage.text`, { val: state.text, ack: true });
        if (state.ts !== undefined)
            await this.setStateAsync(`${baseId}.lastMessage.ts`, { val: state.ts, ack: true });
        if (state.time !== undefined)
            await this.setStateAsync(`${baseId}.lastMessage.time`, { val: state.time, ack: true });
    }
        ...
        for (const candidate of candidates) {
            ...
            if (legitimate.has(baseId)) {
                if (tokens.includes(candidate)) {
                    this.log.warn(`Configured endpoint "${candidate}" matches a token of message archive ${archive.key}. ` +
    ...
    async runExperimentalMessageArchiveWrite(archive) {
        ...
        const report = { key: archive.key, officialRead: {}, experimentalWrite: [], created: false };
        try {
            if (!archive.experimentalWrite)
                throw new Error("Experimenteller Schreibtest ist in der Konfiguration nicht freigegeben.");
            ...
            if (!archive.testToken)
                throw new Error("Kein sicherer Test-Token konfiguriert; es wird kein Token erfunden.");
            if (!initial.tokens.includes(archive.testToken))
                throw new Error(`Test-Token ${archive.testToken} ist nicht in meta vorhanden.`);
            ...
            for (const method of messageArchive_1.EXPERIMENTAL_MESSAGE_ARCHIVE_METHODS) {
                ...
                if (attempt.created) {
                    report.created = true;
                    report.successfulRequest = request;
                    break;
                }
                before = after;
            }
